...
t->mode = NONE - parsing mode: determines what kind of token we’re in
(WORD, SPACE, PIPE, REDIR, QUOTE).
t->modemem = NONE - saves the previous mode so 
the tokenizer can detect when a new token starts 
(e.g. switching from WORD to PIPE).
t->quote = 0 - tracks whether we are inside quotes:
0 -> no quotes, 34 (") -> double quotes, 39 (') -> single quotes.
t->array = ft_calloc(t->maxw + 1, sizeof(char *)) - Allocates memory 
for the token array.
